...
class TwseFetcher:
    def fetch_history_data(self, symbol, start_date=None):
        ...
        if not start_date:
            ...
            date_array = self._get_date_str_for_month(
                start_date
            )  # 根據提供的 start_date 生成資料
        # 日期為陣列，過去6個月的日期
        for date_str in date_array:
            ...
            if not self.check_data_in_db(symbol, date_str):
                ...
                if response.status_code == 200:
                    ...
                    logging_info(f"Data saved for {symbol} on {date_str}.")
                    logging_info(f"Data  {response.json()}")
                else:
                    logging_error(
                        f"Failed to fetch data for {symbol} on {date_str}. Status code: {response.status_code}"
                    )
